...
def period_filter(df,key):
    ...
    if chosen_sub:scoped=scoped[scoped.subcategory.fillna('Sin detalle').isin(chosen_sub)]
...
def vertical_composition(view):
    data=view.groupby(['category','month','month_name'],as_index=False).amount.sum();order=data.groupby('category').amount.sum().sort_values(ascending=False).head(12).index;data=data[data.category.isin(order)];fig=px.bar(data,x='category',y='amount',color='month_name',barmode='group',title='Composición del gasto por mes',category_orders={'category':list(order),'month_name':[MONTHS[m] for m in sorted(data.month.unique())]},color_discrete_sequence=MONTH_COLORS,labels={'amount':'Gasto','category':'Categoría','month_name':'Mes'});fig.update_yaxes(tickprefix='$',tickformat=',.0f');fig.update_xaxes(tickangle=-35);return style(fig)
authenticate()
with st.sidebar:
