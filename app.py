...
def period_filter(df,key):
    years=sorted(df.year.unique(),reverse=True);c1,c2=st.columns([1,3]);year=c1.selectbox('Año',years,key='y'+key);available=sorted(df.loc[df.year==year,'month'].unique());names=[MONTHS[m] for m in available];chosen=c2.multiselect('Meses a analizar',names,default=names,key='months'+key,placeholder='Selecciona uno o varios meses')
    selected=sorted(MONTH_NUM[m] for m in chosen)
    if not selected:st.info('Selecciona al menos un mes para mostrar la información.');st.stop()
...
def stacked_categories(view,title='Gasto por categoría y mes'):
    data=view.groupby(['category','month','month_name'],as_index=False).amount.sum();order=data.groupby('category').amount.sum().sort_values(ascending=False).head(10).index;data=data[data.category.isin(order)].copy();data['category']=pd.Categorical(data.category,categories=list(reversed(order)),ordered=True);data=data.sort_values(['category','month']);fig=px.bar(data,x='amount',y='category',color='month_name',orientation='h',title=title,barmode='stack',text_auto=',.0f',category_orders={'month_name':[MONTHS[m] for m in sorted(data.month.unique())]},color_discrete_sequence=MONTH_COLORS,labels={'category':'Categoría','amount':'Gasto','month_name':'Mes'});fig.update_xaxes(tickprefix='$',tickformat=',.0f');fig.update_traces(texttemplate='$%{x:,.0f}',textposition='inside');return style(fig)

def extraordinary_all():
    base=extraordinary_data().copy();base['year']=2026;base['month_num']=base['month'].astype(str).str.strip().str.title().map(MONTH_NUM);base['source']='Inicial';rows=fetch('extraordinary_expenses')
    if rows:
        saved=pd.DataFrame(rows);saved['expense_date']=pd.to_datetime(saved.expense_date);saved=saved.assign(month=saved.expense_date.dt.month.map(MONTHS),month_num=saved.expense_date.dt.month,year=saved.expense_date.dt.year,source='Agregado').rename(columns={'amount':'amount','concept':'concept'});base=pd.concat([base[['month','concept','amount','month_num','year','source']],saved[['month','concept','amount','month_num','year','source']]],ignore_index=True)
...
if page=='Resumen':
    ...
    with right:
        st.plotly_chart(stacked_categories(view,'Principales categorías por mes'),use_container_width=True)
elif page=='Tendencias y fugas':
    st.title('Tendencias y fugas');st.caption('Evolución, concentración y categorías que más presionan el gasto');view,selected,label=period_filter(monthly,'trend');st.markdown('#### '+label);trend=view.groupby(['month','month_name'],as_index=False).amount.sum().sort_values('month');trend['media']=trend.amount.rolling(3,min_periods=1).mean();fig=go.Figure();fig.add_bar(x=trend.month_name,y=trend.amount,name='Gasto mensual',marker_color=[MONTH_COLORS[m-1] for m in trend.month],text=[money(x) for x in trend.amount],textposition='outside');fig.add_scatter(x=trend.month_name,y=trend.media,name='Promedio móvil 3 meses',line=dict(color=NAVY,width=3),mode='lines+markers');fig.add_hline(y=monthly_budget,line_dash='dash',line_color=GOLD,annotation_text='Presupuesto mensual');fig.update_yaxes(tickprefix='$',tickformat=',.0f');fig.update_layout(title='Gasto y tendencia mensual');st.plotly_chart(style(fig),use_container_width=True);st.plotly_chart(stacked_categories(view,'Composición del gasto por mes'),use_container_width=True);current=view.groupby('category',as_index=False).amount.sum().sort_values('amount',ascending=False);current['share']=current.amount/current.amount.sum();current.columns=['Categoría','Gasto','Participación'];st.subheader('Concentración del gasto');current['Participación']=current['Participación'].map(lambda x:f'{x:.1%}');st.dataframe(current,hide_index=True,use_container_width=True,column_config={'Categoría':st.column_config.TextColumn(width='large'),'Gasto':st.column_config.NumberColumn(format='$ %.2f',width='medium'),'Participación':st.column_config.TextColumn(width='small')})
elif page=='Presupuesto':
    st.title('Presupuesto contra gasto real');st.caption('Comparación de uno o varios meses con categorías conciliadas');view,selected,label=period_filter(monthly,'bud');plan=budget.groupby('category').monthly_budget.sum()*len(selected);actual=view.groupby('category').amount.sum();comp=pd.concat([plan.rename('Presupuesto'),actual.rename('Real')],axis=1).fillna(0);comp['Variación']=comp.Real-comp.Presupuesto;comp['Ejercicio']=comp.Real/comp.Presupuesto.replace(0,pd.NA);comp=comp.reset_index().rename(columns={'category':'Categoría'}).sort_values('Variación',ascending=False);st.markdown('#### '+label);c1,c2,c3=st.columns(3);total_var=float(comp.Variación.sum());c1.metric('Presupuesto',money(comp.Presupuesto.sum()));c2.metric('Gasto real',money(comp.Real.sum()));c3.metric('Variación',money(total_var),delta_color='inverse');display=comp.copy();display['Presupuesto']=display.Presupuesto.map(money);display['Real']=display.Real.map(money);display['Variación']=display.Variación.map(money);display['Ejercicio']=display.Ejercicio.map(lambda x:'—' if pd.isna(x) else f'{x:.1%}');styled=display.style.map(lambda v:f'color:{RED};font-weight:600' if isinstance(v,str) and v.startswith('$') and not v.startswith('$-') else '',subset=['Variación']);styled=styled.map(lambda v:f'color:{GREEN};font-weight:600' if isinstance(v,str) and v.startswith('$-') else '',subset=['Variación']);st.dataframe(styled,hide_index=True,use_container_width=True,column_config={'Categoría':st.column_config.TextColumn(width='large'),'Presupuesto':st.column_config.TextColumn(width='medium'),'Real':st.column_config.TextColumn(width='medium'),'Variación':st.column_config.TextColumn(width='medium'),'Ejercicio':st.column_config.TextColumn(width='small')})
elif page=='Extraordinarios':
    st.title('Gastos extraordinarios');st.caption('Registra gastos fuera del presupuesto ordinario y revisa su evolución mensual');st.metric('Total registrado',money(float(extra.amount.sum())))
    with st.expander('Agregar gasto extraordinario',expanded=False):
        with st.form('extraordinary'):
            c1,c2,c3=st.columns([1,2,1]);date=c1.date_input('Fecha');concept=c2.text_input('Concepto');amount=c3.number_input('Importe',min_value=0.0,step=100.0,format='%.2f');sent=st.form_submit_button('Agregar renglón',type='primary')
        if sent:
            if not concept.strip() or amount<=0:st.error('Escribe un concepto y un importe mayor a cero.')
            else:
                try:insert_one('extraordinary_expenses',{'expense_date':date.isoformat(),'concept':concept.strip(),'amount':amount});st.success('Gasto extraordinario agregado.');st.rerun()
                except Exception as e:st.error(str(e))
    show=extra.sort_values(['year','month_num']).rename(columns={'month':'Mes','concept':'Concepto','amount':'Importe'})[['Mes','Concepto','Importe']];show['Importe']=show.Importe.map(money);show_style=show.style.set_properties(subset=['Importe'],**{'text-align':'center'}).set_table_styles([{'selector':'th.col_heading.level0.col2','props':[('text-align','center')]}]);st.dataframe(show_style,hide_index=True,use_container_width=True,column_config={'Mes':st.column_config.TextColumn(width='small'),'Concepto':st.column_config.TextColumn(width='large'),'Importe':st.column_config.TextColumn(width='medium')})
    chart=extra.groupby(['year','month_num','month'],as_index=False).amount.sum().sort_values(['year','month_num']);chart['Periodo']=chart.apply(lambda r:f"{r['month']} {int(r['year'])}",axis=1);fig=px.bar(chart,x='Periodo',y='amount',title='Gastos extraordinarios por mes',text_auto=',.0f',color='month',color_discrete_sequence=MONTH_COLORS,labels={'amount':'Importe','month':'Mes'});fig.update_yaxes(tickprefix='$',tickformat=',.0f');fig.update_traces(texttemplate='$%{y:,.0f}',textposition='outside');st.plotly_chart(style(fig),use_container_width=True)
