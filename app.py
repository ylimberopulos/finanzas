...
if page=='Resumen':
    ...
    if not investments.empty:
        ...
        if not valuations.empty:
            chart=valuations.merge(investments[['id','label']],left_on='investment_id',right_on='id',how='left');fig=px.line(chart,x='valuation_date',y='value',color='label',markers=True,title='Evolución de las inversiones',labels={'valuation_date':'Fecha','value':'Valor','label':'Inversión'});fig.update_yaxes(tickprefix='$',tickformat=',.0f');st.plotly_chart(style(fig),use_container_width=True)
